...
class GreenWaveCharts extends React.Component {
  constructor(props) {
    ...
    this.reverseWave = []
    this.greenWaveData=this.props.chartsData
  }
  componentDidMount = () => {
    this.prevTime = 0
    ...
    this.getGreenWaveCharts(this.greenWaveData)
  }
                                  ...
                                  <div key={_index} className="phaseBox" style={{ bottom: cycle * _index + 'px' }}>
                                    {
                                      ...
                                      item.phaseList.map((items, indexs) => {
                                        ...
                                        if (item.forward_phase_plan_name === items.phase_name) {
                                          return (
                                            <div key={indexs} id={ reIndex === 1 && _index === 0 ? 'phaseBox' + index : ''} className="greenBox colorBox" phasename={items.phase_name} style={{ height: phaseOffSet * this.Hs + 'px', backgroundColor: '#02EA3C' }}></div>
                                          )
                                        } else {
                                          return (
                                  ...
                                  <div className="phaseBox" style={{ bottom: cycle * _index + 'px' }} key={cycle*_index}>
                                    {
                                      ...
                                      item.phaseList.map((items, indexs) => {
                                        ...
                                        if (item.reverse_phase_plan_name === items.phase_name) {
                                          return (
                                            <div key={items.phase_name} id={ reIndex === 1 && _index === 0 ? 'reversePhaseBox' + index : ''} className="greenBox colorBox" phasename={items.phase_name} style={{ height: phaseOffSet * this.Hs + 'px', backgroundColor: '#02EA3C' }} />
                                          )
                                        }
                                        return (
